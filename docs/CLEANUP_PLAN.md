...
| D6 | Controller class/file naming (`*Controller`, drop redundant prefixes) | Low | Complete |
| D7 | Mapper XML file + namespace standardization (`mapper.{domain}.{feature}`) | Medium | Complete |
| D8 | Gym DAO rename/merge (`GymMain*` → feature DAOs) | Medium | Complete |
| D9 | Member signup controllers (`SignupStep2Controller`, `SignupStep3Controller`) | Low | Complete |
...
### D6 — Controller naming normalization

45 controllers renamed to follow `{Feature}Controller` in `controller.{domain}`:

- Trainer: `Dashboard` → `DashboardController`, `GymLookupServlet` → `GymLookupController`, etc.
- Gym: drop redundant `Gym` prefix (`GymMain` → `MainController`, `GymNoticeController` → `NoticeController`, etc.)
- Admin: `AdminMain` → `MainController`, `ExGuide*` → `ExerciseGuide*Controller`, add `Controller` suffix to `ReportList`, `Sales`, etc.

`@WebServlet` URL patterns unchanged.

### D7 — Mapper XML standardization

- 37 member mapper files renamed from `*Mapper.xml` → `{feature}.xml` (snake_case)
- All namespaces unified to `mapper.{domain}.{feature}` across admin, gym, member, trainer
- Deleted orphans: `gymSchedule.xml`, `TrainerAvailabilityMapper.xml`
- Updated `mybatis-config.xml` and all Java statement ID references

### D8 — Gym DAO rename and merge
...
| `GymMainNoticeDAO` | merged into `NoticeDAO` |
| `GymNoticeDAO` | `NoticeDAO` |
| `GymMainReviewDAO` | merged into `ReviewDAO` |
| `GymReviewDAO` | `ReviewDAO` |
| `GymMainScheduleDAO` | merged into `ScheduleDAO` |
| `GymScheduleDAO` | `ScheduleDAO` |
| `GymMemberManageDAO` | `MemberManageDAO` |
| `GymDashboardDAO` | `DashboardDAO` |
| `InfoEditDAO` | `GymSettingsDAO` |
| `GymTossPaymentDAO` | deleted (uses `dao.common.TossDAO`) |

`GymPaymentDAO` and `GymSalesDAO` keep the `Gym` prefix to avoid clashes with `dao.common.PaymentDAO` and `dao.admin.SalesDAO`.

### D9 — Member signup controller naming

- `Step2Controller` → `SignupStep2Controller`
- `Step3Controller` → `SignupStep3Controller`
- URLs `/member/step2` and `/member/step3` unchanged
