...
public class ReviewDAOImpl implements ReviewDAO {
	...
	public void insertReview(Review review) throws Exception {
		...
		try {
			session.insert("mapper.gym.review.insertReview",review);
			session.commit();
		} catch (Exception e) {
		    session.rollback();
		    throw e;
	...
	public Review selectReviewByReviewNum(int reviewNum) throws Exception {
		SqlSession session = MybatisSqlSessionFactory.getSqlSessionFactory().openSession();
		try {
			return session.selectOne("mapper.gym.review.selectReviewByReviewNum", reviewNum);
		}finally {
			session.close();
		}
	}
	...
	public void updateReview(Review review) throws Exception {
		SqlSession session = MybatisSqlSessionFactory.getSqlSessionFactory().openSession();
		try {
			session.update("mapper.gym.review.updateReview",review);
			session.commit();
		} catch (Exception e) {
		    session.rollback();
		    throw e;
	...
	public void deleteReview(int reviewNum) throws Exception {
		SqlSession session = MybatisSqlSessionFactory.getSqlSessionFactory().openSession();
		try {
			session.delete("mapper.gym.review.deleteReview",reviewNum);
			session.commit();
		} catch (Exception e) {
		    session.rollback();
		    throw e;
	...
	public List<Review> selectReviewListByGymId(int gymId) throws Exception {
		SqlSession session = MybatisSqlSessionFactory.getSqlSessionFactory().openSession();
		try {
			return session.selectList("mapper.gym.review.selectReviewListByGymId", gymId);
		}finally {
			session.close();
		}
	}
